...
// funkziya dlya podseta ozenki i procenta
...
struct Test_type // внутри тест тайпов хранятся вопросы
{
...
struct User_list //имя логин пароль
{
    std::string name;
    std::string surname;
    int telephone = 0;
...
private:
    ...
    User_list* m_user_online = nullptr; // какой user выполняет действия
public:
    ...
    void File_read_question()
    {
        ...
        while (true)
        {
            ...
            {
                type = m_head_test_type = new Test_type;
            }
            ...
            while (true)
            {
                ...
                {
                    type = type->next_test_type = new Test_type;
                    break;
                }
                if (type->question_head == nullptr)
                {
                    q = type->question_head = new Question;
                }
                else
                {
                    q = q->m_next = new Question; // proverit
                }
    ...
    Test_type* choose_type(bool& cancel, bool add = false)
    {
        ...
        {
            system("cls");
            std::cout << *m_head_test_type; // Список test type
...
public:
    ...
    void Modification_user() // zapis v file
    {
        ...
        while (true)
        {
            ...
            if (menu == 2)
            {
                ...
                std::cout << *this; // Список всех user по login
...
#ifdef FILE
                clean_TESTDATA(temp->incomplete_test); // ociwenie pamyati
#endif // FILE
                clean_GRADEDATA(temp); // ociwenie pamyati
    ...
    void Modification_test_type(All_Test* tests_data)
    {
        ...
        while (true)
        {
            ...
            if (menu == 1)
            {
                ...
                while (true)
                {
                    if (tests_data->Get_head_test_type() == nullptr) { tests_data->Set_head_test_type(new Test_type); }
        ...
        while (true)
        {
            ...
            while (true)
            {
                ...
                if (text == "TEST DATA +" || text == "LAST TEST DATA")
                {
                    ...
                    if (temp == nullptr)
                    {
                        temp = user->incomplete_test = new Test_type;
                    }
                    else
                    {
                        while (true)
                        {
                            ...
                            if (temp->next_test_type == nullptr) { temp = temp->next_test_type = new Test_type; break; }
                            temp = temp->next_test_type;
                        }
                    }
                    ...
                    // записывает при выходе из программы, при новом заходе, записывает данные в TEST DATA + и удаляется
    ...
    // параметры работают во время выполнения теста 
    ...
    // type_to_clean - передаём конкретное название type для удаления, когда user его окончательно сдал и он чистит из TEST DATA +
    // если ничего не передать он удалит все незаконченные тесты, передаёт по умолчанию "CLEAN ALL TYPE"
    // когда мы удаляем user с незаконченными тестами, очищает User_list -> Test_type* head
    //
    void clean_TESTDATA(Test_type*& head, std::string type_to_clean = "CLEAN ALL TYPE")
    {
        if (head == nullptr) { return; }
        Test_type* data = nullptr;
        Test_type* temp = head;
        while (type_to_clean == "CLEAN ALL TYPE")
        {
            data = temp->next_test_type;
            clean_QUESTIONDATA(temp);
            delete temp; // ociwenie pamyati
            if (data == nullptr) { head = nullptr; return; }
            temp = data;
        }

        // удаляет конкретный test_type
        if (temp->test_type_name == type_to_clean)
        {
            head = temp->next_test_type;
            clean_QUESTIONDATA(temp);
            delete temp; // ociwenie pamyati
            return;
        }
        while (true)
        {
            if (temp->next_test_type->test_type_name == type_to_clean)
            {
                data = temp;
                temp = temp->next_test_type;
                data->next_test_type = temp->next_test_type;
                clean_QUESTIONDATA(temp);
                delete temp; // ociwenie pamyati
                return;
            }
            if (temp->next_test_type == nullptr) { return; }
            temp = temp->next_test_type;
        }
    }

    void clean_QUESTIONDATA(Test_type* to_clean)
    {
        if (to_clean->question_head == nullptr) { return; }
        Question* data = nullptr;
        Question* temp = to_clean->question_head;
        while (true)
        {
            data = temp->m_next;
            delete temp; // ociwenie pamyati
    ...
    void clean_GRADEDATA(User_list* to_clean)
    {
        if (to_clean->all_grades == nullptr) { return; }
        Grade* data = nullptr;
        Grade* temp = to_clean->all_grades;
        while (true)
        {
            data = temp->m_next_grade;
            delete temp; // ociwenie pamyati
            if (data == nullptr) { return; }
            temp = data;
        }
...
public:
    ...
    ~Authentication()
    {
        User_list* user = m_users_data.Get_head();
        User_list* temp = nullptr;
        Test_type* test = m_tests_data.Get_head_test_type();
        m_users_data.clean_TESTDATA(test);
        while (user != nullptr)
        {
            m_users_data.clean_TESTDATA(user->incomplete_test);
            m_users_data.clean_GRADEDATA(user);
            temp = user;
            user = user->next;
            delete temp;
        }
...
// Dobavleniye user -> (zapis v file user)
std::istream& operator >> (std::istream& in, User& ob)
{
    User_list* temp; // login kotoriy mi vvodim
    User_list* temp2; // proverka na povtor logina
...
// Dobavleniye test type
std::istream& operator >> (std::istream& in, Test_type& ob)
{
    if (ob.test_type_name == "")
    {
        system("cls");
    ...
    last = last->next_test_type = new Test_type;
...
// Dobavleniye test -> (zapis v file question)
std::istream& operator >> (std::istream& in, All_Test& ob)
{
    Question* temp;
    Test_type* type = nullptr;
    ...
    while (true)
    {
        ...
        {
            ob.Set_head_test_type(new Test_type);
...
// Spisok i proxojdeniye test -> (zapis v file user & TEST DATA)
std::ostream& operator << (std::ostream& out, All_Test& ob)
{
    if (ob.Get_head_test_type() == nullptr)
    {
    ...
    while (true)
    {
        ...
        /////////////////////////////////////////////// zapis v fail vo vremya proxojdeniya testa
#ifdef FILE
    ...
    {
        ob.Get_user_online()->all_grades = grade = new Grade;
    }
    else                                              // esli polzovatel uje proxodil test
    {
        while (true)
        {
            ...
            {
                grade = grade->m_next_grade = new Grade;
                break;
            }
            grade = grade->m_next_grade;
        }
    ...
    ///////////////////////////////////////////////////////// zapis v file user & question & ociwenie pamyati
#ifdef FILE
    ob.Get_user()->clean_TESTDATA(ob.Get_user_online()->incomplete_test, type->test_type_name);
